...
int8_t Cross::addStartCar(Map &cityMap, Car* car)
{
...
#if TEST_ANSWER
#else
    if(car->_preset == 0 && car->getNextRoad()->getJams(car->_curCross) > FLOAT_STARTCAR_JAM)
    {
            //car->setStatusStop();
...
#endif
    car->getNextRoad()->addCarToStart(car);
    return 1;
}
