        ...
        for (int i = 0; i < _channel; ++i) {//从第一条车道开始处理
            for (int j = 0; j < carsInRoadFromTo[_channel].size(); ++j) {
                ...
                if (car->_curSpeed > car->_distanceToCross) {
                    ...
                    switch (car->_turnto) {
                        case isLeft:Cross::crosses[next_cross]->outToRoad[_id].LQueue.push(car); break;
                        case isRight:Cross::crosses[next_cross]->outToRoad[_id].RQueue.push(car); break;
                        case isForward:Cross::crosses[next_cross]->outToRoad[_id].DQueue.push(car); break;
                        default:
                            break;
                    }
